...
export class FinderService {
	...
	private lastValidLowest: number | null = null;
	...
	private startSearch() {
		...
		this.lastValidLowest = null;
		this.loggerService.addLog("Starting minimum max buy search...", "info");
		this.performSearch();
	}

	private performSearch(sendSearchEvent = false) {
		if (!this.isSearching) return;
		if (sendSearchEvent) sendPinEvents("Transfer Market Search");

		const searchCriteria = getSearchCriteria();
		const utSearchCriteria = new UTSearchCriteriaDTO();
		Object.assign(utSearchCriteria, searchCriteria);
		utSearchCriteria.maxBuy = this.currentMaxPrice || 0;
	...
	private handleSearchError(response: any): boolean {
		...
		if (
			...
			if (this.consecutiveFailures >= 3) {
				stop = true;
				this.loggerService.addLog(`Search failed ${this.consecutiveFailures} times consecutively - stopping`, "error");
			} else {
	...
	private displayLowestPrices() {
		...
		const sorted = this.lastFoundPrices.sort((a, b) => a - b);
		const lowest = sorted.slice(0, 3);
		lowest.forEach((price) => {
			this.loggerService.addLog(`-> ${price.toLocaleString()} coins`, "success");
		});
	}

	private handleSearchResponse(response: any) {
		if (!this.isSearching) return;
		if (!response.success && this.handleSearchError(response)) return;

		sendPinEvents("Transfer Market Results - List View");

		if (!response.success || !response.data?.items) {
			if (this.lastValidLowest != null) {
				this.loggerService.addLog("No items found, using last valid results", "warning");
				this.lastFoundPrices = [this.lastValidLowest];
		...
		const buyPrices = items
			.filter((i) => i._auction && i._auction.tradeState === "active")
			.map((i) => i._auction.buyNowPrice)
			.filter((p) => p && p >= 200);

		if (buyPrices.length === 0) {
			if (this.lastValidLowest != null) {
				this.loggerService.addLog("No valid buy now prices, reverting to last valid lowest", "warning");
				this.lastFoundPrices = [this.lastValidLowest];
				this.displayLowestPrices();
			} else {
		...
		const lowest = Math.min(...buyPrices);
		this.lastValidLowest = lowest;

		this.loggerService.addLog(`Found ${items.length} items`, "info");

		if (items.length === 21) {
			if (lowest <= 200) {
				this.currentMaxPrice = 200;
				this.loggerService.addLog("Lowest price reached 200. Stopping search.", "info");
				this.displayLowestPrices();
				this.isSearching = false;
				this.updateButtonState();
				return;
			}
			this.currentMaxPrice = getNextLowerValidPrice(lowest);
