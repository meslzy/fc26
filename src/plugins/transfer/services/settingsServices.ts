...
export class SettingsService {
	...
	private createSearchTabContent(): HTMLElement {
		...
		const filterRotationSelect = createSelect({
			label: "Filter Rotation Mode",
			value: this.settingsManager.settings.search.filterRotation,
			options: [
				{ value: "sequential", label: "Sequential (Change every search)" },
				{ value: "per-cycle", label: "Per Cycle (Change after each cycle)" },
				{ value: "random", label: "Random (Pick randomly each search)" },
			],
			onchange: (value) => {
				this.settingsManager.settings.search.filterRotation = value as "sequential" | "per-cycle" | "random";
				this.settingsManager.save();
			},
		});
		...
		container.appendChild(filterRotationSelect.container);
