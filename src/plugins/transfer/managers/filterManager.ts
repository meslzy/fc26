...
export class FilterManager {
	private readonly KEY = "fc26_filters";
	...
	private loadFilters() {
		const savedFilters = localStorage.getItem(this.KEY);
		if (savedFilters) {
	...
	private saveFilters() {
		localStorage.setItem(this.KEY, JSON.stringify(this.filters));
	}

	private getName(searchCriteria: SearchCriteria, playerData: PlayerData | null): string {
		let baseName = "";
	...
	private hasSignificantChange(key: string, value: any, defaultValue: any): boolean {
		const skipFields = ["count", "offset", "_sort", "blockingRequests", "requests"];
	...
	save(searchBucket: SearchBucket) {
		...
		const playerData = getSearchPlayerData();
	...
	select(id: string) {
		...
		setSearch(filter ? filter.searchCriteria : {}, filter ? filter.playerData : null);
	}
	...
	selectAll(searchBucket: SearchBucket) {
		const bucketFilters = this.filters.filter((f) => f.searchBucket === searchBucket);
